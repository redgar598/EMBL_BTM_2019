...
def generate_features_from_fasta(fasta_file,nr_datapoints,kmer_len,outfile):
    ...
    # generate column names from dict
    cc = []
    ks = [k for k, v in kmer_dict.items()]
    ks.sort()
    cn = list(set([v for k, v in kmer_dict.items()]))
    cn = [""] * len(cn)
    for k in ks:
        if kmer_dict[k] not in cc:
            cc.append(kmer_dict[k])
            cn[kmer_dict[k]] = k
    df = pd.DataFrame(contigs,index=contigs_id, columns=cn)
    df.to_csv(outfile)
#    print contigs_id
